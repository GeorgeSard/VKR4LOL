...
from src.preprocessing import FEATURE_COLS, TARGET_DELAY, build_preprocessor

logging.basicConfig(level=logging.INFO, format="%(asctime)s | %(levelname)s | %(message)s")
log = logging.getLogger("train_delay")


def _try_mlflow_log(params: dict, metrics: dict, model: Pipeline, artifacts: list[Path]) -> None:
    try:
        import mlflow
...
def main() -> None:
    ...
    log.info("Обучаю LightGBM (n_estimators=%d)", cfg["n_estimators"])
    ...
    _try_mlflow_log(params, metrics, pipeline, [cm_path, fi_path, metrics_path, metadata_path])
